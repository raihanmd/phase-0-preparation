/**
  Square Number
  Diberikan sebuah function squareNumber2 dimana menerima inputan berupa angka.
...
function squareNumber2(num: number) {
  const res: number[][] = Array.from({ length: num }).map((_) => []);
  let counter = 1;
