...
function minDistanceBetweenGreatest(arr: number[]) {
  let lastGreatestIndex: number = 0;
  let greatest: number = -1;
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] >= greatest) {
      greatest = arr[i];
      result =
        arr[i] === arr[lastGreatestIndex] ? Math.min(i - lastGreatestIndex) : 0;
      lastGreatestIndex = i;
    }
  }
...
console.log(
  minDistanceBetweenGreatest([
    4, 5, 5, 7, 8, 4, 2, 4, 6, 7, 6, 9, 7, 7, 7, 5, 3, 2, 9,
  ]),
); //7
