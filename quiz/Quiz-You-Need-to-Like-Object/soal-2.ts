...
function economyChangeSummary(tradeActivity: string[][]) {
  ...
  for (let i = 0; i < tradeActivity.length; i++) {
    const batch = tradeActivity[i];

    for (let j = 0; j < batch.length; j++) {
      const entry = batch[j]; // e.g. "Jeff Bezos+5%"
      ...
      for (let k = 0; k < entry.length; k++) {
        const ch = entry[k];
        if ((ch === "+" || ch === "-") && k > 0 && operator === "") {
          operator = ch;
          name = entry.substring(0, k);
          for (let m = k + 1; m < entry.length; m++) {
            if (entry[m] !== "%") percentStr += entry[m];
          }
          break;
        }
      }
      ...
      if (name === "Jeff Bezos") {
        if (operator === "+") duitJeff = duitJeff * (1 + percent / 100);
        else duitJeff = duitJeff * (1 - percent / 100);
        deposit = duitJeff;
        owner = "Amazon";
      } else if (name === "Larry Page") {
        if (operator === "+") duitLarry = duitLarry * (1 + percent / 100);
        else duitLarry = duitLarry * (1 - percent / 100);
        deposit = duitLarry;
        owner = "Google";
      } else if (name === "Jack Ma") {
        if (operator === "+") duitJack = duitJack * (1 + percent / 100);
        else duitJack = duitJack * (1 - percent / 100);
        deposit = duitJack;
        owner = "Alibaba";
      }
      deposit = parseFloat(deposit.toFixed(10));
